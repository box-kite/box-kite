...
_16 September 2026 · [npm](https://www.npmjs.com/package/@box-kite/react) · [Compare v1.0.0...v2.0.0](https://github.com/box-kite/box-kite/compare/v1.0.0...v2.0.0)_
...
- **[A popover is a browser feature now, not a portal](#a-popover-is-a-browser-feature-now-not-a-portal)** — `<Popover>` on the platform Popover API: the top layer, light dismiss and focus return are the browser's, so there is no portal and no z-index — and it is 3.46 KB gz on top of Box.
...
- **[A modal dialog is a browser feature too](#a-modal-dialog-is-a-browser-feature-too)** — `<Dialog>` and `<AlertDialog>` on the native `<dialog>`: `showModal()` supplies the top layer, the backdrop, an inert page, Escape, focus containment and focus return, so the two together add 2.15 KB gz on top of Box.
- **[A menu, and its submenus, are nested popovers](#a-menu-and-its-submenus-are-nested-popovers)** — `<Menu>` is APG's menu button on the Popover API: seven parts, the whole keyboard, submenus that nest as deeply as the markup, and 6.36 KB gz on top of Box.
- **[Tabs, where selection follows focus](#tabs-where-selection-follows-focus)** — the APG tabs pattern in four parts: one tab stop for the list, arrows that follow the reading order, both activation modes, and 3.03 KB gz on top of Box. An indicator that travels between tabs and a panel box that resizes between panels are each one opt-in away.
- **[An accordion whose animation is a class](#an-accordion-whose-animation-is-a-class)** — `<Accordion>` and `Collapsible`, where a panel opens in a grid track rather than a measured pixel: no `ResizeObserver`, nothing written per instance, and 1.80 KB gz for both.
- **[A slider whose value keeps its own shape](#a-slider-whose-value-keeps-its-own-shape)** — `<Slider>` takes a number for one thumb and an array for a range, so nothing at the call site has to narrow; `<Progress>` is the bar beside it, and renders on a server. 2.14 KB gz and 0.31 on top of Box.
- **[A message you send rather than render](#a-message-you-send-rather-than-render)** — `<Toaster />` once, then `toast()` from anywhere at all: a live region that exists before there is anything in it, a queue rather than a cap, and timers that stop on hover, on focus and off screen. 4.14 KB gz on top of Box, styled.
- **[A combobox whose value is your own row](#a-combobox-whose-value-is-your-own-row)** — the pattern most libraries leave you to assemble: `<Combobox>` takes your rows and hands one back, the filter composes, the selection can be chips, a query nothing answers can become a row, and a list of ten thousand opens in one frame. 9.53 KB gz on top of Box.
...
- **[The grid is one tab stop, the way APG says](#the-grid-is-one-tab-stop-the-way-apg-says)** — everything a DataGrid draws inside a cell leaves the tab sequence, so crossing a grid is one press rather than seventy-eight: Enter or F2 steps in, Escape hands the keyboard back, Space selects the row, and `def.tabNavigation="cells"` is the other reading.

## The package tells an agent how to use it

A coding agent has priors about a library this new, and they are wrong: the prop names collide with other libraries' while the numbers mean something else, so the failure mode is code that compiles and is laid out wrong. The cheapest fix is a file in the tree it already greps, so the tarball carries one:
...
It is also **small**. `<Popover>` adds **3.46 KB gz** on top of a `Box` an app already has, and 1.03 KB where `Tooltip` or `Dropdown` already ship the placement and behaviour chunks it shares. Measured, minified and gzipped.
...
It is also **small**: `<Dialog>` and `<AlertDialog>` together add **2.15 KB gz** on top of a `Box` an app already has, because the pattern is the browser's. Measured, minified and gzipped.
...
It is **small**: `<Menu>` and all seven of its parts add **6.36 KB gz** on top of a `Box` an app already has. Measured, minified and gzipped.
...
It is **small**: `<Tabs>` and all four of its parts add **3.03 KB gz** on top of a `Box` an app already has, measured, minified and gzipped. About 1.16 KB of that is the two things that move, and it is there whether or not they are switched on: `Tabs.Panels` hangs off `Tabs` and the list measures from the same module, so neither shakes out.
...
It is **small**: `<Accordion>`, all three of its parts and `Collapsible` add **1.80 KB gz** on top of a `Box` an app already has. Measured by a harness that ships in the repo — `npm run size:published`, so the number in this paragraph can be re-derived rather than believed. Measuring nothing is most of what a panel animation usually costs.
...
`<Slider>` is APG's slider and its multi-thumb sibling in one component, and `<Progress>` is the bar beside it. **2.14 KB gz and 0.31 KB gz** on top of Box — 2.31 for both together, measured the same way.
...
**4.14 KB gz** on top of Box, styled — no stylesheet to import, because the appearance is in the JavaScript already counted. Measured as what it adds to an app that already has Box. The style tree behind it also costs **0.50 KB gz** on every entry that carries the engine, whether or not the app renders a toast: the built-in component styles ship with Box, and this is the largest of them.
...
**9.53 KB gz on top of Box** — the model, the listbox, the chips, the windowing and the APG keyboard, on top of the anchoring and dismissal every other layer in the library already shares. A combobox is usually assembled instead — a headless behaviour package, and then the markup, the styling and the ARIA written around it.
...
none of it and the grid's own entry grows 1.03 KB for the model and the buttons. A real `.xlsx` is
usually either a paid tier or a second dependency; here it is neither.
...
two style-tree nodes — `/a11y` and `/anchor`, which carry no engine, moved nothing at all.
...
exactly zero, which is what attributes the rest.
...
opened and then waited for a second press to show what could be chosen is the interaction a select
editor
...
inside that cell, and **Escape** hands the keyboard back. That is APG's reading of a grid
...
next cell and on into the rows after it, Shift+Tab walks back, the way a spreadsheet reads a grid. It is
...
select-all box sits in, it toggles every row. That is the spreadsheet reading, and it is the half that makes
