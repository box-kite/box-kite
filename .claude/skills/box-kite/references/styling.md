...
ecosystem already uses, so a chart lifted from shadcn's charts works unchanged. It adds no role and no ARIA, and the palette it declares is its own `chartContainer` style node — override that and every chart on the page follows.
