...
export default class TreeModel<TRow> {
  ...
  /**
   * Whether the grid is a tree. A datasource answers its own rows, so it wins: lazy children are the
   * datasource's own contract rather than something to run over a `data` array that is never filled.
   */
  public get enabled(): boolean {
    return !!this.config && !this.grid.source.enabled;
  }
  ...
  /** Whether ticking a row ticks everything under it. */
  public get cascades(): boolean {
    return this.config?.selection === 'cascade';
  }
  ...
  public readonly nodes = memo(() => {
    if (!this.enabled) return [] as TreeNode<TRow>[];
  ...
  public toggle = (key: Key, level: number): void => {
    ...
    this.grid.rows.clear(); // cascades to flatRows/rowOffsets
