import { LAYOUT_SCHEMA, LAYOUT_VERSION, MAX_COLUMNS as MAX, MIN_COLUMNS as MIN } from './dashboardSchema';
...
namespace DashboardUtils {
  // Declared beside the schema that quotes them, so the bounds a generated layout is clamped to and the
  // bounds it is described by cannot drift apart.
  export const VERSION = LAYOUT_VERSION;
  export const MIN_COLUMNS = MIN;
  export const MAX_COLUMNS = MAX;
  ...
  /** The layout as JSON Schema: what a model generates a dashboard under, and what `<SpecRenderer>` validates. */
  export const SCHEMA = LAYOUT_SCHEMA;
}
